...
**Status**: Plan only (実装は別 session、user 確認後)
**Estimated effort**: 30-50h initial + 5-10h/week ongoing maintenance

## Phase 0: Pre-flight (user 確認事項)

実装着手前に user 承認が必要な項目:

- [ ] GitHub repo 作成: `kazuyahibara/claude-second-brain-jp` (public, MIT)
- [ ] AgriciDaniel/claude-obsidian の fork 方針: hard fork or attribution-only adaptation
- [ ] CC plugin marketplace 登録 (Anthropic 公式 marketplace への submit、レビュー期間あり)
- [ ] `claude-second-brain-jp.com` 等の docs site ドメイン (任意)

## Phase 1: Fork & local clone

```bash
# 1. AgriciDaniel/claude-obsidian を fork (GitHub UI or gh)
gh repo fork AgriciDaniel/claude-obsidian --fork-name=claude-second-brain-jp --clone

cd claude-second-brain-jp
git remote add upstream https://github.com/AgriciDaniel/claude-obsidian.git

# 2. attribution / license 整理
# - README.md: 「Forked from AgriciDaniel/claude-obsidian (MIT)」を冒頭に明記
# - LICENSE: MIT 維持、Copyright 行に Kazuya Hibara を追加
# - .claude-plugin/plugin.json: author / homepage / repository / keywords を JP 向けに更新
```

## Phase 2: 7 JP additions

### 2.1: 5-layer hot cache
...
## Phase 4: Karpathy compliance gap fix

AgriciDaniel に対して spec から外れている点を修復:

### 4.1: Sources rewrite 強度
